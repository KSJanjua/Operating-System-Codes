    ...
    public:
        ...
        void calculate_values(processes* proc_arr, int num_pro)
        {
            ...
            while(complete<num_pro)
            {
                ...
                {
                    if (time>=proc_arr[j].arrival_time && (min==-1 || remaining_time[min]>remaining_time[j]) && remaining_time[j]>0 )
                        min=j;
                }
                ...
                {
                    proc_arr[min].completion_time=time;
                    proc_arr[min].turnaround_time=proc_arr[min].completion_time-proc_arr[min].arrival_time;
                    proc_arr[min].waiting_time=proc_arr[min].turnaround_time-proc_arr[min].burst_time;
                    complete++;
                }
            }
        }
