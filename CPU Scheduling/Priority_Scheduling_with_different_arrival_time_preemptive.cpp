//Lower the priority number assigned higher the priority of the process
    ...
    processes()
        ...
        void calculate_values(processes* proc_arr, int num_pro)
        {
            ...
            while (complete<num_pro)
            {
                ...
                {
                    if (time>=proc_arr[j].arrival_time && (pro_ind==-1 || proc_arr[pro_ind].priority>proc_arr[j].priority) && remaining_time[j]>0)
                        pro_ind=j;
                }

                time++;
                remaining_time[pro_ind]--;
                if (remaining_time[pro_ind]==0)
                {
