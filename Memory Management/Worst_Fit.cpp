...
class process
{
    ...
    public:
        ...
        void worst_fit_memory_alloc(memory* memo_arr, int num_blo)
        {
            int max=INT_MIN;
            int ID=-1;
            for (int j=0; j<num_blo; j++)
            {
                ...
                {
                    if (max<memo_arr[j].block_size)
                    {
                        max=memo_arr[j].block_size;
                        ID=j;
                    }
                }
            }
        ...
        void display_data()
        {
            ...
            cout << "No memory block allocated to process " << pro_ID << " with size " << process_size << endl ;            
        }
};
...
int main()
{
    ...
    for (int i=0; i<num_pro; i++) 
        (proc_arr+i)->worst_fit_memory_alloc(memo_arr, num_blo);
