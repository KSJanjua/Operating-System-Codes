...
class process
{
    ...
    public:
        ...
        void first_fit_memory_alloc(memory* memo_arr, int num_blo)
        {
            bool done=0;
            for (int j=0; j<num_blo; j++)
            {
                if (process_size<=memo_arr[j].block_size && memo_arr[j].available==1)
                {
                    allocated=memo_arr[j].block_ID;
                    memo_arr[j].available=0;
                    break;
                }
            }
        }
...
int main()
{
    ...
    for (int i=0; i<num_pro; i++) 
        (proc_arr+i)->first_fit_memory_alloc(memo_arr, num_blo);
